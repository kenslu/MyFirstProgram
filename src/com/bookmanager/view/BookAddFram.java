...
public class BookAddFram {

	public static void bookAddFram() {
			
			JFrame frame = new JFrame("图书添加界面");
			...
			//提示框
			JLabel label1 = new JLabel("添 加 界 面");
			...
			button1.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					...
					try {
						...
						ResultSet currentId = bookdao.bookCheck(con, id);
						if(currentId.next()) {
							JOptionPane.showMessageDialog(null, "图书Id已存在，请重新输入Id！");
							return;
						}
						
						//若不存在，可以进行插入
						int current = bookdao.add(con,bookMessage);
